...
isolated_quic_topo( fd_drv_t * drv, fd_topo_obj_callbacks_t * callbacks[]  ) {
  ...
  ulong tile_to_cpu[ FD_TILE_MAX ] = {0}; // same cpu0 for testing
...
FOR(quic_tile_cnt) fd_topob_link( topo, "quic_net", "quic", config->net.ingress_buffer_size, FD_NET_MTU, 64 );
...
FOR(net_tile_cnt) fd_topob_tile_in( topo, "quic", 0UL, "metric_in", "net_quic", 0UL, FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED );
