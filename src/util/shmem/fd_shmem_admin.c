...
# define ERROR( cleanup ) do { err = errno; goto cleanup; } while(0)
  ...
  FD_LOG_WARNING(("shmem private path %s", p));
  ...
  FD_LOG_WARNING(("cane allocating size: %lu", sz));

  shmem = mmap( NULL, sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_HUGETLB | MAP_HUGE_2MB, fd, (off_t)0);
  ...
  FD_LOG_WARNING(("cane: shmem %p, page %lu", shmem, page_sz));
...
# define ERROR( cleanup ) do { err = errno; goto cleanup; } while(0)
  ...
  /* fallocate the file, to force the kernel to allocate disk space. This is done because otherwise,
     if the disk runs out of space accessing the mapped memory will throw SIGBUS mid-execution.
     It's better to fail early, if the disk is not large enough to back the whole memory region. */
  if( FD_UNLIKELY( fallocate( fd, FALLOC_FL_KEEP_SIZE, 0, (off_t)sz ) ) ) {
    FD_LOG_WARNING(( "fallocate(\"%s\",FALLOC_FL_KEEP_SIZE,%lu KiB) failed (%i-%s)",
                     path, sz>>10, errno, fd_io_strerror( errno ) ));
    ERROR( unmap );
  }
