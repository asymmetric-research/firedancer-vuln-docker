...
main( int argc, char ** argv ) {
  ...
  char const * _page_sz  = fd_env_strip_cmdline_cstr  ( &argc, &argv, "--page-sz",        NULL, "huge"                   );
  ulong        page_cnt  = fd_env_strip_cmdline_ulong ( &argc, &argv, "--page-cnt",       NULL, 100UL                          );
  ...
  fd_wksp_t * wksp = fd_wksp_new_anonymous( page_sz, page_cnt, fd_shmem_cpu_idx( numa_idx ), "wksp_client", 0UL );
  FD_TEST( wksp );

  fd_quic_limits_t quic_limits = {0};
  fd_quic_limits_from_env( &argc, &argv, &quic_limits );
  quic_limits.conn_cnt = 2;
  quic_limits.handshake_cnt = 6;
  quic_limits.tx_buf_sz = 2048;
  quic_limits.stream_pool_cnt = 4;
